...
4. **render阶段**：render阶段的主角是`Reconciler`，render阶段的主要工作是**构建Fiber树**和生成**effectList**。
...
5. **commit阶段**：会遍历`EffectList`，处理相应的生命周期，将这些**副作用应用到真实节点**，这个过程会对应不同的渲染器，在浏览器的环境中就是react-dom，在canvas或者svg中就是reac-art等。 
