...
1. 浏览器首先查看查找本地`hosts文件`（将一些常用的网址域名与其对应的IP地址建立一个关联数据库），如果有则返回，否则进行下一步。
2. 浏览器查看`本地缓存`，是否存在本条目的缓存，如果有则直接返回，否则进行下一步。
3. 将请求转发给`本地域名服务器`LDNS来解析这个域名。在本地域名服务器上查看域名 是否本地解析，是则本地解析返回，否则在`本地DNS服务器的缓存`中查找，有则返回，无则进行下一步
4. 向全球13个`根域服务器发`起DNS请求，
5. 根域名服务器返回给本地域名服务器LDNS一个所查询域的`主域名服务器`（（gTLD Server，国际顶尖域名服务器，如.com .cn .org等））地址
6. 此时本地域名服务器LDNS再发送DNS请求给`主域名服务器gTLD`
7. 接受请求的gTLD查找并返回这个域名对应的`Name Server`（网站注册的域名服务器）的地址给本地域名服务器，这个Name Server就是网站注册的域名服务器
8. 本地域名服务器再向这个`网站注册的域名服务器`发送DNS请求，Name Server根据映射关系表找到目标ip，返回给本地域名服务器LDNS
9. LDNS`缓存`这个域名和对应的ip
10. LDNS把解析的结果返回给`浏览器`，浏览器并缓存到本地系统缓存中，域名解析过程至此结束