...
PS：如果需要包含 `cookie` 信息，ajax 请求需要设置 xhr 的属性 `withCredentials(表示跨域请求时是否携带身份凭证)` 为 true，服务器需要设置响应头部 `Access-Control-Allow-Credentials`: true。
...
1. **对脚本进行过滤或转码**
2. 充分利用 `CSP`，CSP 的本质是建立一个白名单，告诉浏览器哪些外部资源可以加载和执行，从而防止恶意代码的注入攻击。开启CSP的方式有：
