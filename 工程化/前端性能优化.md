...
- [性能优化分类及优化方案](#性能优化分类及优化方案)
  ...
  - [运行时性能优化](#运行时性能优化)
    ...
    - [使用 Web Workers优化JS长任务](#使用-web-workers优化js长任务)
    - [写代码时的优化点](#写代码时的优化点)
- [Webpack](#webpack)
...
#### 实现
- `React.lazy` + `import()` + `Suspense`:React利用 React.lazy与import()实现了渲染时的动态加载 ，并利用Suspense来处理异步加载资源时页面应该如何显示的问题。Suspense可以在数据加载时暂停视图更新，直到异步操作完成后再继续渲染
  ```js
...
function debounce(fn, wait) {
  ...
  return function() {
    ...
    // 设置定时器，使事件间隔指定事件后执行
    timer = setTimeout(() => {
      fn.apply(this, args); // 将我们执行回调方法时的上下文绑定在回调函数上，apply会立即执行
    }, wait);
  };
}
