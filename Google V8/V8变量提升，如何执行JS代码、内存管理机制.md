...
4. **预解释/代码生成** - 生成字节码：字节码是介于 AST 和机器代码的中间代码。解释器**可执行字节码**。这个阶段会进行`变量的提升`。
...
> 2. 当解释器执行到热点代码时，会优先执行编译成机器代码后的热点代码
>   * 热点代码是指被重复多次执行的代码。
...
1. **创建堆空间和栈空间（栈溢出错误是什么？）**
   ...
   * **函数**调用过程中，涉及到**上下文**相关的内容都会存放在栈上，   如**  原生类型**引用到的对象的地址、函数的执行状态、this 值等都   存在在栈上。
...
# 延迟解析：V8如何实现闭包？
**惰性解析**：解析器在解析的过程中，如果遇到函数声明，那么会跳过函数内部的代码，并不会为其生成 AST 和字节码，而仅仅生成顶层代码的 AST 和字节码。