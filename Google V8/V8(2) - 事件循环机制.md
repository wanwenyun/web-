...
>`setTimeout`函数用于在指定的延迟时间后执行一次任务。
>`setInterval`函数用于以固定时间间隔重复执行任务。
...
const async1 = async () => {
    ...
    await new Promise(resolve => {
        console.log('promise1')
    }) // 只有被Promise.resolve()了的，await后面的语句才会被执行
    console.log('async1 end') // 不执行
    return 'async1 success' // 不执行
} 
console.log('script start');
...
Promise.resolve(1)
    ...
    // .catch()方法返回的仍然是一个Promise对象，所以可以链式调用.then()方法 
    // 输出是1.因为promise的状态一旦被流转，就无法改变，所以在最初被resolve成1之后，结果一直为1，永不变
setTimeout(() => {
    console.log('timer2')
}, 1000)
```
