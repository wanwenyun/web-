...
4. 为什么需要再搭建 BFF 层？因为由前端同学再去维护一个 BFF 层的服务是有人力输出的？是有哪些特殊的诉求吗？
    ...
    因为后台的服务是trpc服务，不能直接用http来调用。搭建BFF层实际上就是做了http请求的代理转发。并且在这里做了一些接口聚合，数据整合，智能网关身份校验（因为是内网系统，所以可以直接获取用户身份信息）等操作。再加上，老板希望我们拓宽自身的知识面，所以选择了用node来搭建该项目的BFF层。

    <img src="./picture/business.png"/>

5. BFF 接口的 QPS （每秒请求量）有多大？应该不算是高并发的内部系统？
   
    是个内部系统，并发量不大。

6. 前端项目的构建打包工具用的是 webpack 还是 cra 还是其它的公司内部的工具？

    我们的前端项目是用的公司内的的桌面React框架，TDesign。它集成了vite来进行打包。

7. BFF 层的服务部署在公司内部的容器云中，是用的 pm 2 来作为 node 的进程管理工具吗？node服务在容器云是用什么方式起动的？
...
10. 从0到1搭建node，强调如何快速上手nest.js，突出学习能力。
    请求、日志、告警、鉴权
